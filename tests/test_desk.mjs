...
const { STAMPS, pressed } = await import(path.join(root,'js/stamps.js'));
...
t('six stamps, and pressing DOI does NOT mint', () => {
  assert.equal(STAMPS.length, 6);
  const r = pressed('doi');
  assert.ok(r.ok);
  says(r.note, 'Nothing has been minted');
  says(r.note, 'that is a separate gate');
  assert.equal(STAMPS.find(s=>s.id==='doi').mints, false); });
t('no stamp claims to mint', () => {
  for (const s of STAMPS) assert.equal(s.mints, false, s.id+' claims to mint'); });
