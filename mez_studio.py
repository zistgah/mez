...
"""mez studio — the spine.
    ...
    mez studio mount <id>            how to reach one sovereign system
    mez studio doctor                what is present, what is absent, what is a wiring gap

A LABORATORY, NOT A CONTAINER
-----------------------------
The systems it reaches EXIST INDEPENDENTLY. GENIE runs without Mez; Kitab runs without Mez; Mez
runs without either. This is a table of interfaces, not an inventory of parts — it says how a
sovereign system can be reached, exercised and composed from here, and nothing more.

Composition is not absorption. Two systems may compose into something new without either being
modified, and where a reusable primitive falls out it is published back into the ecosystem as its
own thing rather than folded into this bench.
...
def registry(path=None):
    ...
    if not os.path.exists(p):
        raise SystemExit("components.json not found at %s — there is no interface table to read." % p)
...
def mount_url(c, s):
    ...
    if not s["live"] and c["reached_by"] == "serve":
        return None
    if c["reached_by"] == "serve" and s["live"]:
        return "http://127.0.0.1:%d/%s" % (c["port"], c.get("path", "").lstrip("/"))
    if c["reached_by"] in ("static", "dome") and s["path"]:
...
def how(c, s):
    ...
    elif c["reached_by"] == "serve" and not s["live"]:
...
def state(reg):
    ...
    for c in reg["components"]:
        s = status(c)
        comps.append({**{k: c[k] for k in ("id", "title", "group", "what", "repo", "reached_by", "rungs")},
...
def cmd_mount(reg, cid):
    ...
    print("repo %s · mount %s · %s" % (c["repo"], c["reached_by"], s["why"]))
    print("")
...
def cmd_dome(reg):
    c = next((x for x in reg["components"] if x["reached_by"] == "dome"), None)
    if not c:
...
def cmd_doctor(reg):
    ...
    idle = [(c, s) for c, s in rows if s["status"] == "present" and c["reached_by"] == "serve"]
...
def selftest():
    ...
    if all(c.get("sovereign") for c in r["components"]) and all("reached_by" in c for c in r["components"]):
        ok("every system is sovereign and REACHED, never contained")
    else: bad("a component is described as owned rather than reached")
    arch = " ".join(r.get("_architecture", [])).lower()
    if "laboratory, not a container" in arch and "synthesis does not mean incorporation" in arch:
        ok("the registry states federation, not containment, in its own text")
    else: bad("the architecture note does not state independence")
    ...
    fake = {"rungs": r["rungs"], "components": [
        {"id": "ghost", "title": "Ghost", "group": "x", "what": "not here",
         "repo": "nowhere/ghost", "reached_by": "static", "path": "index.html",
         "rungs": ["ide"], "sovereign": True}]}
    ...
    srv = next(c for c in r["components"] if c["reached_by"] == "serve")
